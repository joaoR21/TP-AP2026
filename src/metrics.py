...
def f1_macro(y_true, y_pred):
    def correct_format(y):
        if len(y[0]) == 1:
            return np.array([np.round(y[i][0]) for i in range(len(y))])
        else:
            return np.array([np.argmax(y[i]) for i in range(len(y))])
    ...
    for c in classes:
        ...
        f1s.append(2 * precision * recall / (precision + recall + 1e-10))
    return np.mean(f1s)
